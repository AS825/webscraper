...
def process_flights(driver, wait):
    ...
    for row in rows:
        ...
        # wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, '.row.gx-1')))
...
def extract_and_print_flight_details(row, previous_hour,flight_details):
    ...
    if previous_hour[0] == 23 and (current_hour == 6 or current_hour == 7):
        return False

    previous_hour[0] = current_hour
    flight = row.find_element(By.CSS_SELECTOR, '.col-3.col-lg-2').text[:6]
    destination = row.find_element(By.CSS_SELECTOR, '.d-lg-none').text
    flugzeugtyp_label = row.find_element(By.XPATH, ".//dt[contains(text(), 'Flugzeugtyp:')]")
    airplane_type = flugzeugtyp_label.find_element(By.XPATH, "./following-sibling::dd").text
    flight_info = {"Time": time_str, "Flight":flight, "From": destination, "Plane": airplane_type}
    flight_details.append(flight_info)
    print(flight_info)

    return True

def scrape_depatures():
    website = 'https://www.viennaairport.com/passagiere/ankunft__abflug/abfluege'
    ...
    try:
        ...
        with open('flight_data_Departure.json', 'w') as file:
            json.dump(flights, file, indent=4) 
        print("Flight data saved to 'flight_data_Departure.json'")
